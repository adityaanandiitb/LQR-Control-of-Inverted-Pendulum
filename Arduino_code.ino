...
#define dir1 26
#define dir2 24
//long theta, alpha, prev_theta = 0, prev_alpha = 0; // LONG?
//double dtheta,dalpha, currentTime, prevTime, output;
//float theta0;
//const float K[] = { -7.0711   ,57.0050,   -2.5352 ,   7.2612};
void setup() 
{
...
void loop() 
{
  ...
  double dt;
  double modo,max1;
  double K[]={-7.51879699248072 , 142.898666477457,  -4.67006186366597, 16.7794291146019};
//   -7.2886  ,146.3252  , -4.6575 ,  16.774 more besttttt
  // -7.5188 , 142.8987  , -4.67071  , 16.7794 new besht
// -7.1429 , 146.2328 ,  -4.6185 ,  16.6911  fst
  // -6.66666666666666 ,241.487296676908  ,-11.0832634667653 ,31.4957778041353 khaas nahi 
  double theta_eq=9290*(PI/8192); 
  double alpha_eq=8250*(PI/8192); 
  int pid_val; 
...
//    if (encoderPosition0 == 0xFFFF) //position is bad, let the user know how many times we tried
//    {
...
//      Serial.print(encoderPosition0, DEC); //print the position in decimal format
//      Serial.write(NEWLINE);
//    }
...
//
//    if (encoderPosition1 == 0xFFFF) //position is bad, let the user know how many times we tried
//    {
...
//      Serial.println(encoderPosition0); //print the position in decimal format
//      Serial.write(NEWLINE);
//    }
    ...
    prev_wakt=wakt;
//    Serial.print("prev=");
//    Serial.println(prev_wakt);
    wakt=(double)millis(); // typecast
//    Serial.println(wakt);
...
//    Serial.println(dt);
    ...
    output=-1*(K[0]*(theta-theta_eq)+K[1]*(alpha-alpha_eq)+K[2]*theta_dot+K[3]*alpha_dot);
////    output=output;
////    Serial.print("Output: ");
    modo=(abs(output));
...
//      Serial.println(encoderPosition0);
 //1 -> arm
//    delay(100); // no delay *_*  ^o^
  }
}
 ...
 * f
 * or position verification. Both 12-bit and 14-bit encoders transfer position via two bytes, giving 16-bits regardless of resolution.
